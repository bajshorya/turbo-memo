...
export function Controls() {
  ...
  const agents = [
    { value: "volume-analyser", label: "Volume Analyser", category: "Volume Analyser" },
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Combobox value={selectedAgent} onValueChange={handleAgentChange}>
              <ComboboxTrigger className={`h-12 min-h-12 border-gray-200 rounded-full px-4 font-medium ${
                selectedAgent 
              ...
              <ComboboxContent className="bg-white border-gray-200 rounded-xl shadow-lg">
                <ComboboxList>
                  {agents.map((agent) => (
                    <ComboboxItem key={agent.value} value={agent.value} className="text-[#473c75] focus:bg-gray-50">
                      <div>
